...
export async function fetchDatasetDates() {
  try {
    const [income, birth, hdb, coe, ges, climate] = await Promise.all([
    ...
    return {
      income: formatYear(income?.result?.records?.[0]?.Dollar),
      birth: birthLatest,
      hdb: formatMonth(hdb?.result?.records?.[0]?.month),
      coe: formatMonth(coe?.result?.records?.[0]?.month),
      ges: formatYear(ges?.result?.records?.[0]?.year),
      climate: formatMonth(climate?.result?.records?.[0]?.month)
    };
  } catch (e) {
    console.error('Error fetching dates:', e);
    return { income: 'Updated', birth: 'Updated', hdb: 'Updated', coe: 'Updated', ges: 'Updated', climate: 'Updated' };
  }
}
